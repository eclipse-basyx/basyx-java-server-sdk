...
Operation delegation now validates outbound delegation targets with secure defaults.
This example delegates to the Docker-internal host example-operation-service:8080, so the environment configuration includes an explicit allowlist entry in [examples/BaSyxOperationDelegation/basyx/aas-env.properties](examples/BaSyxOperationDelegation/basyx/aas-env.properties).
